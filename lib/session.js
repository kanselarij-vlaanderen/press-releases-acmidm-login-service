...
const GROUP_ID_CLAIM = process.env.MU_APPLICATION_AUTH_GROUPID_CLAIM || 'vo_orglijst';
...
const ensureUserGroup = async function(claims) {
  let voOrganization = null; // e.g. "Departement Omgeving:OVO003323"
  if (claims[GROUP_ID_CLAIM] instanceof Array) {
    // TODO workaround until configuration has been changed at ACM/IDM.
    // We should receive only 1 organization for a logged in user.
    // For now, we select the first organization from the list.
    voOrganization = claims[GROUP_ID_CLAIM][0];
  } else {
    voOrganization = claims[GROUP_ID_CLAIM];
  }
  const index = voOrganization?.lastIndexOf(':');
  if (index >= 0) {
    const organizationName = voOrganization.substring(0, index);
    const organizationIdentifier = voOrganization.substring(index + 1);
    ...
    FROM <${PUBLIC_GRAPH_URI}> {
      ?group a foaf:Group ;
            mu:uuid ?groupId ;
            dcterms:identifier ${sparqlEscapeString(organizationIdentifier)} .
    }`);
    ...
    if (queryResult.results.bindings.length) {
      ...
      const { groupUri, groupId } = await insertNewUserGroup(organizationIdentifier, organizationName);
...
const insertNewUserGroup = async function(identifier, name) {
  const groupId = uuid();
  ...
  let insertData = `
    ...
    INSERT DATA {
      GRAPH <${PUBLIC_GRAPH_URI}> {
        ${sparqlEscapeUri(group)} a foaf:Group, vcard:Organization ;
                                 mu:uuid ${sparqlEscapeString(groupId)} ;
                                 vcard:fn ${sparqlEscapeString(name)} ;
